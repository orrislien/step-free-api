...
var ServiceConfig = class _ServiceConfig {
  ...
  constructor(options) {
    ...
    this.name = import_lodash3.default.defaultTo(name, "step-free-api");
...
var MODEL_NAME = "step";
var ACCESS_TOKEN_EXPIRES = 900;
var MAX_RETRY_COUNT = 0;
var RETRY_DELAY = 5e3;
var FAKE_HEADERS = {
  Accept: "*/*",
...
async function requestToken(refreshToken) {
  ...
  const result = await (async () => {
    const result2 = await import_axios.default.post(
...
function messagesPrepare(convId, messages, refs) {
...
`;
    ...
    return content2 += `${message.role || "user"}:${message.content}
`;
  }, "") + "assistant:";
  logger_default.info("\n\u5BF9\u8BDD\u5408\u5E76\uFF1A\n" + content);
  const json = JSON.stringify({
    ...
    messageInfo: {
      text: content,
      attachments: refs.length > 0 ? refs : void 0
    }
  });
...
function checkResult(result, refreshToken) {
  ...
  if (!import_lodash12.default.isString(code))
    return result.data;
  if (code == "unauthenticated")
    accessTokenMap.delete(refreshToken);
  throw new APIException(exceptions_default.API_REQUEST_FAILED, `[\u8BF7\u6C42step\u5931\u8D25]: ${message}`);
}
async function receiveStream(model, convId, stream) {
  return new Promise((resolve, reject) => {
    ...
    let refContent = "";
    const parser = (buffer) => {
      const result = import_lodash12.default.attempt(() => JSON.parse(buffer.toString()));
      if (import_lodash12.default.isError(result)) {
        logger_default.warn(`Error response: ${buffer.toString()}`);
      ...
      if (result.error && result.error.code)
        ...
        if (result.pipelineEvent.eventSearch && result.pipelineEvent.eventSearch.results) {
...
function generateCookie(deviceId, accessToken) {
...
async function uploadFile(fileUrl, refreshToken) {
  ...
  const startTime = util_default.unixTimestamp();
  while (needFurtherCall) {
    result = await import_axios.default.post(
    ...
    if ([12, 22, 59, 404].includes(fileStatus))
      throw new APIException(exceptions_default.API_FILE_UPLOAD_FAILED);
    if (util_default.unixTimestamp() - startTime > 60)
      throw new APIException(exceptions_default.API_FILE_UPLOAD_TIMEOUT);
  }
...
(async () => {
  ...
  logger_default.info("<<<< step free server >>>>");
